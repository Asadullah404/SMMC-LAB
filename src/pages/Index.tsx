...
import { useToast } from "@/hooks/use-toast";
import { LabReport } from "@/types/lab-report";
import { Microscope, Moon, Sun, User, LogOut, Search } from "lucide-react";
import { format } from "date-fns";
...
const Index = () => {
  const { toast } = useToast();
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [currentStep, setCurrentStep] = useState<"patient" | "tests" | "preview">("patient");
  const [user, setUser] = useState<any>(null);
  const [role, setRole] = useState<string | null>(null);
  const navigate = useNavigate();
  ...
  const saveReportToFirebase = async () => {
    ...
    try {
      ...
      toast({ title: "Report Saved", description: "The report was saved successfully." });
    } catch (error) {
      console.error(error);
      toast({ title: "Error", description: "Failed to save report." });
    }
  ...
  return (
    <div className="min-h-screen bg-background transition-medical flex flex-col">
      ...
      <header className="border-b bg-card card-shadow sticky top-0 z-50">
        ...
        {showMobileSearch && (
          <div className="sm:hidden bg-card border-t px-4 py-3 animate-slideDown">
        ...
        {currentStep === "tests" && (
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              <Button onClick={() => setCurrentStep("preview")}>Generate Report</Button>
            </div>
          </div>
        )}
